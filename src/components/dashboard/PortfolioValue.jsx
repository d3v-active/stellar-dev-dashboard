...
import { TrendingUp, TrendingDown, PieChart as PieChartIcon, Activity, AlertTriangle, Target, DollarSign } from 'lucide-react'
import Card, { StatCard } from './Card'

const CHART_COLORS = ['#00d4ff', '#00ff88', '#ff6b6b', '#ffd93d', '#a78bfa', '#fb923c', '#ec4899', '#14b8a6']
  ...
  return (
    <Card title="Portfolio Analytics" subtitle="Comprehensive portfolio analysis">
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        {!pricesLoading && activeView === 'risk' && analytics && (
          <RiskView analytics={analytics} />
        )}
      </div>
    </Card>
  )
...
function OverviewView({ portfolio, analytics }) {
  if (!portfolio || !analytics) {
    return <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '20px' }}>No data available</div>
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
        ...
        <StatCard
          ...
          sub={diversificationScore >= 7 ? 'Well diversified' : diversificationScore >= 4 ? 'Moderate' : 'Concentrated'}
          accent="var(--purple)"
        />
        <StatCard
          label="Risk Level"
          ...
          accent={riskAssessment.level === 'Low' ? 'var(--green)' : riskAssessment.level === 'Medium' ? 'var(--yellow)' : 'var(--red)'}
        />
      </div>
      ...
      <Panel title="Asset Holdings">
        ...
        {items.map((item, i) => (
          ...
          >
            ...
            <span style={{
              textAlign: 'right',
              color: item.change24h !== null ? (item.change24h >= 0 ? 'var(--green)' : 'var(--red)') : 'var(--text-muted)',
          ...
          <BarChart data={assetPerformance} layout="vertical">
            ...
            <Bar dataKey="change" fill="var(--cyan)">
              {assetPerformance.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={entry.change >= 0 ? 'var(--green)' : 'var(--red)'} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {riskAssessment.recommendations.map((rec, i) => (
              <div key={i} style={{
                ...
                <Target size={14} style={{ color: 'var(--cyan)', marginTop: '2px', flexShrink: 0 }} />
